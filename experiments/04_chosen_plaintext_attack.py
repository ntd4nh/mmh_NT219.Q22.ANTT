"""
Experiment 04: Chosen-Plaintext Attack on AES
================================================
Train DL models to predict the key byte using differential analysis.

The attacker chooses specific plaintexts, encrypts them, and observes
ciphertext differences to deduce key information.

Input: 48 bytes (plaintext1 + ciphertext1 + ciphertext_diff)
...
from data.synthetic.generator import ChosenPlaintextDataset
...
def parse_args():
    parser = argparse.ArgumentParser(description="Chosen-Plaintext Attack Experiment")
    parser.add_argument("--rounds", type=int, default=3, help="Number of AES rounds")
    ...
    parser.add_argument("--sweep-rounds", action="store_true",
                        help="Run for rounds 1-6 and plot accuracy vs rounds")
    parser.add_argument("--output-dir", type=str, default="./artifacts/results/chosen_plaintext")
...
def create_dataloaders(args, num_rounds=None):
    """Generate chosen-plaintext dataset."""
    if num_rounds is None:
        num_rounds = args.rounds
    
    print(f"\n📦 Generating chosen-plaintext dataset ({num_rounds} rounds)...")
    
    total_samples = args.train_samples + args.val_samples + args.test_samples
    
    dataset = ChosenPlaintextDataset(
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % 5 == 0 or epoch == 1:
            ...
            print(f"  Epoch {epoch:3d}/{args.epochs} | "
                  ...
                  f"Best: {best_val_acc*100:.2f}% | Time: {elapsed:.0f}s")
...
def run_single_experiment(args):
    ...
    print(f"\n{'#'*60}")
    print(f"# Chosen-Plaintext Attack — AES ({args.rounds} rounds)")
    ...
    # ---- Train CNN (input_size=48 for chosen-plaintext) ----
    if args.model in ["cnn", "both"]:
        cnn = SmallCNN(input_size=48, num_classes=256, dropout=0.3)
        acc, hist, preds, labels = train_and_evaluate(
            cnn, "SmallCNN", train_loader, val_loader, test_loader, args, device
        )
        results['SmallCNN'] = acc
        print_metrics_summary(preds, labels,
                              experiment_name=f"SmallCNN (Chosen-PT, {args.rounds}R)")
        ...
        plot_training_curves(
            ...
            title=f"SmallCNN — Chosen-Plaintext {args.rounds}-Round AES"
        )
    
    # ---- Train Transformer (input_size=48) ----
    if args.model in ["transformer", "both"]:
        tf = CryptoTransformer(input_size=48, num_classes=256,
        ...
        print_metrics_summary(preds, labels,
                              experiment_name=f"Transformer (Chosen-PT, {args.rounds}R)")
        
        plot_training_curves(
            hist['train_loss'], hist['val_loss'], hist['train_acc'], hist['val_acc'],
            save_path=os.path.join(args.output_dir, f"tf_r{args.rounds}_curves.png"),
            title=f"Transformer — Chosen-Plaintext {args.rounds}-Round AES"
        )
    
    # ---- Model Comparison ----
    if len(results) >= 2:
        results['Random Baseline'] = 1.0 / 256
        plot_model_comparison(
            results, metric_name='Test Accuracy',
            save_path=os.path.join(args.output_dir, "model_comparison.png"),
            title=f"Model Comparison — Chosen-Plaintext {args.rounds}-Round AES"
        )
    
    # Save results
    summary = {
        'experiment': 'chosen_plaintext',
        'num_rounds': args.rounds,
        'target_byte': args.target_byte,
...
def run_round_sweep(args):
    """Run experiment across rounds 1-6 to show security degradation."""
    device = get_device(args.device)
    print(f"\n{'#'*60}")
    print(f"# Chosen-Plaintext Attack — Round Sweep (1-6)")
    print(f"{'#'*60}")
    ...
    plot_accuracy_vs_rounds(
        ...
        title="Chosen-Plaintext Attack: Accuracy vs AES Rounds"
    )
